...
static int verify_expected_subject(SSL *ssl, const char *expected_cn)
{
    ...
    const char *raw = (const char *)ASN1_STRING_get0_data(data);

    
    size_t len = strnlen(raw, sizeof(cn) - 1);
    snprintf(cn, sizeof(cn), "%.*s", (int)len, raw);

    X509_free(cert);

    if (strncmp(cn, expected_cn, sizeof(cn)) != 0) {
...
int main()
{
    ...
    chat_serv_addr.sin_addr.s_addr = inet_addr(server_ip); /* user entered IP */
    chat_serv_addr.sin_port        = htons(server_port);   /* user entered port */
    ...
    while (!response){
        ...
        if (fgets(input, sizeof(input), stdin) != NULL) {
            ...
            if (sscanf(input, "%s", username) == 1) {
                size_t ulen = strnlen(username, MAX);
                if (ulen < 1 || ulen > 31) {
